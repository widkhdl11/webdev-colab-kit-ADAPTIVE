...
| 3 활동 | `report/activity.jsonl` | `.claude/hooks/report-activity.mjs` (PostToolUse) | 툴이 돌 때마다. 토큰 0 |
| 3' 서브에이전트 | `report/subagents.jsonl` | `.claude/hooks/report-subagent.mjs` | 서브에이전트 시작·종료. 토큰 0 |
...
3층(활동)과 3'층(서브에이전트)을 켜려면 `.claude/hooks/` 에 훅 파일이 있어야 하고
`.claude/settings.json` 에 그 명령이 들어가야 한다. 이 레포에서 그 둘은 **사용자가 직접
고치는 자리**다.
