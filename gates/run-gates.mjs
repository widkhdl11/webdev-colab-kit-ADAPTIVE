...
// 1''') 클라이언트 번들에 서버 전용 모듈이 딸려 들어가나.
//      판정은 scripts/check-client-server-import.mjs 한 자리에 있다 — 여기서 로직을 베끼지 않고
//      그것을 돌린다. 베끼면 씨앗 목록(next/headers·server-only)이 늘 때 한쪽만 늘고,
//      게이트와 검사가 다른 답을 낸다.
//
//      **--quick 에도 넣는다(전체 실행 전용이 아니다).** 이 위반은 유닛 검사가 전부 초록불이고
//      `npm run build` 에서만 죽는다. 전체 실행에만 달면 지금과 달라지는 게 없다 —
//      study-mate 에서 2026-09-05~06 에 네 번 났고 매번 잡은 것은 빌드 한 번이었다.
//      훑는 데 0.1초라 편집마다 돌아도 된다.
//
//      검사기가 없거나 못 돌면 죽지 않고 ⚠ 로 건너뛴다. 부분 적용 상태가 편집을 막지 않게.
{
  const bundleCheck = join(ROOT, "scripts", "check-client-server-import.mjs");
  if (!existsSync(bundleCheck)) {
    console.error("⚠ [bundle/SKIP] scripts/check-client-server-import.mjs 가 없다 — 클라이언트/서버 경계 검사를 건너뛴다.");
  } else {
    // **--scan 으로 부른다.** 인자 없이 부르면 검사기가 자기 픽스처까지 돌리는데, 그중 하나가
    // run-gates 를 부른다 — 게이트→검사→게이트로 서로를 부르며 끝나지 않는다.
    const br = spawnSync(process.execPath, [bundleCheck, "--scan"], { cwd: ROOT, encoding: "utf-8" });
    const bout = (br.stdout ?? "") + "\n" + (br.stderr ?? "");
    if (br.status === null) {
      console.error("⚠ [bundle/SKIP] 클라이언트/서버 경계 검사가 실행되지 않았다 — 나머지 게이트는 그대로 판정한다.");
    } else if (br.status !== 0) {
      // 검사기가 낸 줄을 그대로 올린다 — 어느 파일 몇 번째 줄인지와 어디를 거쳐 닿았는지가 그 줄에 있다.
      const hits = bout.split("\n").filter((l) => l.trim().startsWith("[bundle/"));
      if (hits.length) for (const line of hits) errors.push(line.trim());
      else errors.push("[bundle/CLIENT_IMPORTS_SERVER] 클라이언트/서버 경계 검사가 실패했는데 이유 줄이 없다 — 'node scripts/check-client-server-import.mjs --scan' 으로 직접 본다");
    }
  }
}
...
// ── 훅 배선: 보호 훅이 어느 작업 폴더에서도 도는가. (전체 실행 전용)
//    판정은 scripts/check-hooks.mjs 한 자리에 있다 — 여기서 로직을 베끼지 않고 그것을 돌린다.
//
//    **왜 게이트에 올리나.** 훅 실패는 non-blocking 이라 도구가 그대로 실행된다. 훅 명령이
//    상대 경로면 작업 폴더가 projects/<이름> 으로 옮겨가는 순간(npm 명령 하나면 그렇게 된다)
//    시크릿·보호 파일·위험 명령 차단과 편집 게이트와 Stop 훅이 **한꺼번에 조용히 꺼지는데
//    겉모습이 똑같다.** 2026-09-06 에 실제로 한 세션이 통째로 그 상태였고, 화면에 뜬 오류
//    줄을 사람이 알아봐서 잡혔다 — 그것 말고는 알아차릴 방법이 없었다.
//
//    **--wiring-only 로 부른다.** 그냥 부르면 검사기가 훅 프로세스를 오십 번 넘게 띄운다.
//    느린 게이트는 결국 빠지고, 그러면 이 자리를 다시 아무도 안 보게 된다.
//
//    **--quick 에 넣지 않는다.** 편집마다 돌 필요가 없다 — settings.json 은 보호 파일이라
//    편집으로 바뀌지 않고 사람이 직접 고칠 때만 바뀐다.
//
//    검사기가 없거나 못 돌면 죽지 않고 ⚠ 로 건너뛴다. 부분 적용 상태가 편집을 막지 않게.
if (!QUICK) {
  const hooksCheck = join(ROOT, "scripts", "check-hooks.mjs");
  if (!existsSync(hooksCheck)) {
    console.error("⚠ [hooks/SKIP] scripts/check-hooks.mjs 가 없다 — 훅 배선 검사를 건너뛴다.");
  } else {
    const hr = spawnSync(process.execPath, [hooksCheck, "--wiring-only"], { cwd: ROOT, encoding: "utf-8" });
    const hout = (hr.stdout ?? "") + "\n" + (hr.stderr ?? "");
    if (hr.status === null) {
      console.error("⚠ [hooks/SKIP] 훅 배선 검사가 실행되지 않았다 — 나머지 게이트는 그대로 판정한다.");
    } else if (hr.status !== 0) {
      // 검사기가 낸 실패 줄(✗)을 그대로 올린다 — 무엇이 어떻게 어긋났는지가 그 줄에 있다.
      const bad = hout.split("\n").filter((l) => l.includes("✗")).map((l) => l.trim());
      if (bad.length) for (const line of bad) errors.push(`[hooks/WIRING] ${line.replace(/^✗\s*/, "")}`);
      else errors.push("[hooks/WIRING] 훅 배선 검사가 실패했는데 이유 줄이 없다 — 'node scripts/check-hooks.mjs' 로 직접 본다");
    }
  }
}

// ── 검사 등록부: scripts/check-*.mjs 가 자기 역할을 선언했고, 그 선언이 배선과 맞나. (전체 실행 전용)
//    판정은 scripts/check-registry.mjs 한 자리에 있다 — 여기서 로직을 베끼지 않고 그것을 돌린다.
//
//    **왜 게이트에 올리나.** 2026-09-14 에 검사 스물아홉 개의 호출처를 전부 훑었더니 게이트가
//    매번 부르는 것은 다섯이었다(client-server-import·cycle-policy·exempt-expiry·hooks·mirror-sync).
//    나머지 스물넷은 사람이 손으로 쳐야만 돈다. 그중 무엇이 "매번 돌아야 하는데 안 도는 것"이고
//    무엇이 "한 번 쓰고 끝난 도구"인지 파일만 봐서는 구별되지 않는다 — 그래서 검사를 만들었다는
//    기록(LESSONS.md 열다섯 군데)이 그 검사가 지금도 돈다는 뜻처럼 읽힌다.
//
//    **--changed 로 부른다.** 선언·배선 대조만 하는 게 아니라, 이번에 바뀐 하네스 파일을 지키는
//    on-change 검사까지 실제로 돌린다. 보통 0건이고 gates/ 를 고친 턴에만 한둘이 돈다.
//
//    **--quick 에 넣지 않는다.** 편집마다 하위 프로세스를 띄울 일이 아니다.
//
//    배선 대기(pending) 건수는 오류가 아니라 알림으로 올린다. 전부 오류로 띄우면 상수가 된
//    경고가 되어 아무도 안 본다 — 이 레포가 이미 두 번 겪은 모양이다.
//
//    검사기가 없거나 못 돌면 죽지 않고 ⚠ 로 건너뛴다. 부분 적용 상태가 편집을 막지 않게.
if (!QUICK) {
  const registryCheck = join(ROOT, "scripts", "check-registry.mjs");
  if (!existsSync(registryCheck)) {
    console.error("⚠ [registry/SKIP] scripts/check-registry.mjs 가 없다 — 검사 등록부를 건너뛴다.");
  } else {
    const rr = spawnSync(process.execPath, [registryCheck, "--changed"], { cwd: ROOT, encoding: "utf-8" });
    const rout = (rr.stdout ?? "") + "\n" + (rr.stderr ?? "");
    for (const line of rout.split("\n")) {
      if (line.startsWith("▦") || line.trim().startsWith("· check-")) console.error(line);
    }
    if (rr.status === null) {
      console.error("⚠ [registry/SKIP] 검사 등록부가 실행되지 않았다 — 나머지 게이트는 그대로 판정한다.");
    } else if (rr.status !== 0) {
      // 검사기가 낸 줄을 그대로 올린다 — 어느 파일이 무엇을 어겼는지가 그 줄에 있다.
      const bad = rout.split("\n").filter((l) => l.trim().startsWith("[registry/"));
      if (bad.length) for (const line of bad) errors.push(line.trim());
      else errors.push("[registry/FAIL] 검사 등록부가 실패했는데 이유 줄이 없다 — 'node scripts/check-registry.mjs' 로 직접 본다");
    }
  }
}
