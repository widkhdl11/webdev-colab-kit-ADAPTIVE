...
describe("upsertBatches", () => {
  it("INV-S5 실패경로: 본문이 빈 항목은 content_html 을 페이로드에 넣지 않는다", () => {
    ...
    const [batch] = upsertBatches([draft({ contentHtml: "" })], NOW, RUN_ID);
    expect(batch).toBeDefined();
    expect(Object.keys(batch![0]!)).not.toContain("content_html");
  });

  it("INV-S5: 본문이 있으면 넣는다", () => {
    const [batch] = upsertBatches([draft({ contentHtml: "<p>본문</p>" })], NOW, RUN_ID);
    expect(batch![0]!.content_html).toBe("<p>본문</p>");
  });

  it("출처 요약글도 같다 — 없을 때 null 로 덮지 않는다", () => {
    const [batch] = upsertBatches([draft({ sourceExcerpt: null })], NOW, RUN_ID);
  ...
  it("INV-S2 (S19): 출처 요약글이 있으면 넣는다", () => {
    ...
    const [batch] = upsertBatches([draft({ sourceExcerpt: "출처가 준 글" })], NOW, RUN_ID);
  ...
  it("항목의 필수 컬럼이 빠지지 않는다 (INV-C2 원본 URL · INV-C5 대체 표시)", () => {
    ...
    // 여기서 서명을 통째로 고정한다.
    const [batch] = upsertBatches([draft()], NOW, RUN_ID);
  ...
  it("2026-08-17: 실행 id 를 항상 함께 싣는다 (소스별 대시보드가 이 값으로 고른다)", () => {
    const [batch] = upsertBatches([draft()], NOW, RUN_ID);
    expect(batch![0]!.last_ingest_run_id).toBe(RUN_ID);
  });
  ...
  it("INV-O2: 주소 근거는 적재할 때마다 다시 싣는다", () => {
    ...
    const [batch] = upsertBatches([draft({ officialBasis: "byUrl" })], NOW, RUN_ID);
  ...
  it("INV-O2 실패경로: none 은 페이로드에 넣지 않는다 (내용 근거를 지우면 안 된다)", () => {
    ...
    const [batch] = upsertBatches([draft({ officialBasis: "none" })], NOW, RUN_ID);
  ...
  it("항목이 없으면 배치도 없다 (빈 upsert 를 부르지 않는다)", () => {
    expect(upsertBatches([], NOW, RUN_ID)).toEqual([]);
  });
});
