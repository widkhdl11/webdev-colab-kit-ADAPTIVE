...
export interface BudgetReport {
  ...
  /**
   * 키워드 단계 **안에서** 멈춘 건수. 단계를 통째로 안 돌린 `skippedKeywords` 와 다른 칸이다
   * (2026-09-23, INV-CB10·CB12) — 섞으면 "예산이 아예 없었다"와 "여든 건 중 스물넷에서
   * 멈췄다"가 같은 모양이 되고, 이어달리기 판정이 그 둘을 갈라 봐야 한다.
   */
  skippedKeywordItems: number;
  /** 핫이슈 단계 안에서 멈춘 건수. 위와 같은 이유로 `skippedHotIssue` 와 따로 둔다. */
  skippedHotIssueItems: number;
  /**
   * 후보를 받아 오는 자리에서 **잘렸나** (2026-09-23, INV-CB10).
   *
   * 건수 상한을 없앤 뒤로 한 바퀴가 보는 전부가 이 풀이다. 여기서 잘리면 화면 밖에 더
   * 있는데 **예산은 하나도 안 썼으므로** 다른 칸은 전부 0 이다. 이 칸이 없으면 그 상태가
   * 「다 했다」와 같은 모양이 된다.
   */
