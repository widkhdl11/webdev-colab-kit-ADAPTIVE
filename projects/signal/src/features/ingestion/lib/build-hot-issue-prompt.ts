...
export function buildHotIssuePrompt({
  ...
  rules.push(
    DATA_BOUNDARY,
    // signal 포인트 근거 (INV-G2, 2026-09-23) — 상세 화면에 「왜 신호인가」로 선다.
    // 같은 호출에서 받는다: 따로 부르면 같은 글을 두 번 보내 요금이 두 배다.
    "참(true)으로 답한 질문마다 **근거** 에 한 문장을 쓴다. 이 글을 읽는 사람 기준으로 왜 참인지 쓰고" +
      "(예: \"장시간 도는 에이전트의 API 비용이 크게 줄어든다.\"), 조건이 붙으면 문장 안에 쓴다" +
      "(예: \"WordPress 사이트를 운영한다면 테마 디렉터리를 지금 점검해야 한다.\"). 60자 안팎으로 짧게 쓰고 마침표로 끝낸다. " +
      "거짓인 질문에는 근거를 쓰지 않는다.",
    "출력은 JSON 객체 하나뿐이다. 예: " +
      '{"종류": ["뉴스"], "변화": true, "방향": false, "기회": true, ' +
      '"근거": {"변화": "…한다.", "기회": "…한다."}, "같은사건": false}. ' +
      "다른 말은 쓰지 않는다.",
  );
