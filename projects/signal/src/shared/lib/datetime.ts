...
/**
 * 날짜 키(`YYYY-MM-DD`) → 그 날이 시작하는 순간의 UTC ISO 시각. `dayKey` 의 역함수다.
 * 키가 날짜 꼴이 아니면 null — 부르는 쪽이 "경계를 못 정했다"로 다룬다.
 *
 * `dayKey` 옆에 두는 이유: 둘이 같은 오프셋을 써야 "이 날짜에 속한 것"과 "이 시각 이후"가
 * 같은 집합이 된다. 떨어뜨려 놓으면 한쪽만 시간대를 바꿔도 아무 신호 없이 어긋난다.
 */
export function dayStartIso(
  ...
  return Number.isNaN(t) ? null : new Date(t - offsetMinutes * MIN_MS).toISOString();
}
