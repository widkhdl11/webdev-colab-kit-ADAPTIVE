...
export function markTrending<T extends Rankable>(
  ...
  /**
   * 그룹 전체를 받지 못한 날짜 키. 그 날에는 뱃지를 **하나도** 안 붙인다 (INV-R5).
   *
   * INV-R5 는 뜨는 중을 「그 날 전체에서 점수 상위 N」으로 정의한다. 일부만 받은 그룹에서
   * 상위 N 을 뽑으면 그건 다른 기준이고, 틀렸다는 표시가 화면에 안 난다 —
   * 2026-09-19 리뷰가 잡은 것이 그 자리다. 없는 뱃지는 눈에 띄지만 틀린 뱃지는 안 띈다.
   */
  excludeDayKeys: readonly string[] = [],
): (T & { isTrending: boolean })[] {
  const trending = new Set<string>();
  const excluded = new Set(excludeDayKeys.filter((k) => k !== ""));
