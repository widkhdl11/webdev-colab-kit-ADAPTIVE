...
describe("markTrending — INV-R5 날짜 그룹 안 상위 N", () => {
  ...
  describe("실패 경로", () => {
    ...
    // 2026-09-19 리뷰: 조회가 하루를 한가운데에서 자르면 그 그룹의 상위는 「그 날의 상위」가
    // 아니다. 틀린 뱃지는 화면에 표시가 안 나고, 없는 뱃지는 눈에 띈다.
    it("INV-R5 실패경로: 전체를 못 받은 날짜는 그 그룹 전체가 뜨는 중에서 빠진다", () => {
      expect(trendingIds(markTrending(items, 3, ["2026-08-09"]))).toEqual(["d2-1", "d2-2"]);
    });

    it("INV-R5 프로브: 뺀 날짜 말고는 그대로다 — 하나를 빼는 것이 전부를 지우지 않는다", () => {
      expect(trendingIds(markTrending(items, 3, ["2026-08-08"])))
        .toEqual(["d1-1", "d1-2", "d1-3"]);
    });

    it("INV-R5 프로브: 빈 목록을 넘기면 아무것도 안 빠진다 — 기본값이 전부를 빼지 않는다", () => {
      expect(trendingIds(markTrending(items, 3, [])))
        .toEqual(["d1-1", "d1-2", "d1-3", "d2-1", "d2-2"]);
    });
  });
});
