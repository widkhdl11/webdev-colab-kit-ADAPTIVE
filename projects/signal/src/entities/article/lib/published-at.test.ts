...
describe("normalizePublishedAt — INV-C5 발행시각 UTC 정규화", () => {
  ...
  /**
   * 2026-08-26: 아이타임스·인공지능신문이 타임존 없이 한국시간 벽시계값을 준다
   * (`2026-08-26 17:50:06`). 이걸 UTC 로 읽으면 9시간 미래가 되어 ① 날짜가 하루 밀려 보이고
   * ② 랭킹의 나이가 0 이라 피드 맨 위에 계속 고정된다. 실측으로 29건이 미래에 있었다.
   */
  describe("타임존 없는 값을 소스의 시간대로 읽는다 (2026-08-26)", () => {
    it("INV-C5: 한국 매체의 존 없는 시각은 KST 로 읽어 UTC 로 바꾼다", () => {
      // 실제로 잘못 저장됐던 값. 한국시간 17:42 → UTC 08:42 여야 한다.
      const r = normalizePublishedAt("2026-08-26 17:42:25", FETCHED_AT, "Asia/Seoul");
      expect(r.publishedAt).toBe("2026-08-26T08:42:25.000Z");
      expect(r.isFallback).toBe(false);
    });

    it("INV-C5: 시간대를 안 주면 예전대로 UTC 로 읽는다 (나머지 12곳의 동작이 안 바뀐다)", () => {
      expect(normalizePublishedAt("2026-08-26 17:42:25", FETCHED_AT).publishedAt).toBe(
        "2026-08-26T17:42:25.000Z",
      );
    });

    it("INV-C5: 날짜만 있는 값도 그 시간대의 00:00 으로 읽는다", () => {
      // KST 자정 = 전날 15:00 UTC. UTC 로 읽었으면 09-01T00:00Z 였다.
      const r = normalizePublishedAt("2026-09-01", FETCHED_AT, "Asia/Seoul");
      expect(r.publishedAt).toBe("2026-08-31T15:00:00.000Z");
    });

    it("INV-C5: 시간대를 줘도 타임존이 **명시된** 값은 그 타임존을 따른다", () => {
      // 소스 설정은 "존이 없을 때 무엇으로 볼지"만 정한다. 존이 적혀 있으면 그게 우선이다 —
      // 안 그러면 같은 피드에 섞여 오는 정상 표기까지 9시간 틀어진다.
      expect(
        normalizePublishedAt("2026-08-26T17:42:25Z", FETCHED_AT, "Asia/Seoul").publishedAt,
      ).toBe("2026-08-26T17:42:25.000Z");
      expect(
        normalizePublishedAt("2026-08-26T17:42:25-04:00", FETCHED_AT, "Asia/Seoul").publishedAt,
      ).toBe("2026-08-26T21:42:25.000Z");
    });

    it("INV-C5: 오프셋이 다른 시간대도 그 시간대만큼 옮긴다 (KST 만 되는 게 아니다)", () => {
      // +9 를 하드코딩하면 이 줄이 빨간불이 된다.
      expect(
        normalizePublishedAt("2026-08-26 12:00:00", FETCHED_AT, "America/New_York").publishedAt,
      ).toBe("2026-08-26T16:00:00.000Z"); // EDT(-4)
      expect(
        normalizePublishedAt("2026-08-26 12:00:00", FETCHED_AT, "Europe/Berlin").publishedAt,
      ).toBe("2026-08-26T10:00:00.000Z"); // CEST(+2)
    });

    it("INV-C5: 일광절약시간이 있는 지역은 그 날짜의 실제 오프셋을 쓴다", () => {
      // 같은 벽시계값이 여름/겨울에 다른 UTC 가 된다. 고정 오프셋으로 계산하면 한쪽이 틀린다.
      expect(
        normalizePublishedAt("2026-01-15 12:00:00", FETCHED_AT, "America/New_York").publishedAt,
      ).toBe("2026-01-15T17:00:00.000Z"); // EST(-5)
      expect(
        normalizePublishedAt("2026-07-15 12:00:00", FETCHED_AT, "America/New_York").publishedAt,
      ).toBe("2026-07-15T16:00:00.000Z"); // EDT(-4)
    });

    it("INV-C5 실패경로: 모르는 시간대 이름이면 대체한다 — 조용히 UTC 로 읽지 않는다", () => {
      // UTC 로 흘려보내면 설정 오타가 9시간 오차로 조용히 남는다. 대체는 리포트에 남는다.
      const r = normalizePublishedAt("2026-08-26 17:42:25", FETCHED_AT, "Asia/Seuol");
