...
const row = {
  ...
  item_tag: [
    { tag: { name: "MCP", axis: "field" } },
    { tag: { name: "출시", axis: "kind" } },
  ],
};
...
describe("toStoredArticle — 조회 경계", () => {
  it("행을 도메인 모양으로 옮긴다", () => {
    ...
    // 이름만이 아니라 **축까지** 실려 와야 한다 — 화면이 두 축을 색으로 가른다.
  ...
  it("고정 5개 목록으로 거르지 않는다 — 모델이 만든 말이 그대로 온다", () => {
    // 여기가 자유 생성 키워드를 통째로 버리던 세 자리 중 하나였다(2026-08-30).
    // `ARTICLE_TAGS.includes` 로 걸러서 저장은 됐는데 화면에는 안 나왔다.
    const a = toStoredArticle({
      ...row,
      item_tag: [
        { tag: { name: "온디바이스AI", axis: "field" } },
        { tag: { name: "규제", axis: "kind" } },
      ],
    });
    expect(a?.tags).toEqual([
      { name: "온디바이스AI", axis: "field" },
      { name: "규제", axis: "kind" },
    ]);
  });

  it("축이 없거나 모르는 값이면 그 뱃지만 버린다 — 색을 못 정하는 뱃지는 안 그린다", () => {
    const a = toStoredArticle({
      ...row,
      item_tag: [
        { tag: { name: "코딩", axis: "field" } },
        { tag: { name: "옛태그", axis: "legacy" } },
        { tag: { name: "축없음" } },
        { tag: null },
      ],
    });
    expect(a?.tags).toEqual([{ name: "코딩", axis: "field" }]);
  });
