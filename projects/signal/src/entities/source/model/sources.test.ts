...
describe("SOURCES — 목록의 모양", () => {
  ...
  it("INV-F4: 선별 판정을 거는 소스가 정확히 어디인지 못 박는다", () => {
    // 강제 위치가 **소스 설정**이라 파이프라인 테스트로는 못 잡는다.
    // 값이 바뀌면 사람이 한 번 더 판단하라는 뜻의 테스트다 — 2026-08-13 에 실제로 그렇게 걸렸다.
    //
    // **긍정 쪽으로 적는다** (2026-08-16 감사): `filter(s => !s.needsTopicCheck)` 가 `[]` 인지
    // 보는 식은 "예외가 없다"만 붙들고 **판정을 거는 곳이 누구인지는 아무것도 안 적는다.**
    // 그러면 목록이 통째로 비거나 id 가 오타로 바뀌어도 그대로 green 이라, 옆 테스트의
    // 개수 검사에 기대야만 뜻이 산다. 이름을 적어 두면 소스 추가·삭제·오타가 전부 diff 에 뜬다.
    //
    // 하나라도 false 로 내리면 **그 소스의 글은 쓸모 축을 통째로 건너뛴다** —
    // 사용자가 걸러 달라고 한 세 글이 정확히 그렇게 들어와 있었다(aitimes·etnews).
    expect(SOURCES.filter((s) => s.needsTopicCheck).map((s) => s.id)).toEqual([
      "openai-blog",
      "google-ai-blog",
      "deepmind-blog",
      "huggingface-blog",
