...
//
// **needsTopicCheck 는 2026-08-13 에 전 소스 true 가 됐다.** 전에는 geeknews·hn-frontpage
// 두 곳뿐이었고, 근거는 "AI 전용 피드에 '이게 AI 글인가'를 묻는 건 답이 정해진 질문"이었다.
// 그 근거는 판정이 **소재만** 물을 때만 성립한다. 같은 날 선별 기준에 쓸모 축이 붙으면서
// (prompt-text.ts TOPIC_SCOPE) AI 전용 피드에서도 답이 정해지지 않는 질문이 생겼다 —
// 실제로 사용자가 걸러 달라고 한 세 글이 전부 판정을 안 걸던 소스(aitimes·etnews)에서 왔다.
//
// 비용 — **실측은 신규 건수뿐이고 시간은 산정이다.** (2026-08-16 리뷰에서 정정)
//   실측: 하루 신규 약 68건 (2026-08-13 수집으로 612→680건). 이미 적재된 항목은 판정을
//         건너뛴다(run-ingest.ts 의 listKnownUrls).
//   산정: 건당 3.8초(2026-08-12 실측: 70건 순차 264초) ÷ 동시 8건.
//   **가정이 붙는다**: 동시성 8은 *소스 안에서만* 걸린다. 신규가 소스별로 8건 이내이고
//         목록 앞쪽에 몰려 있을 때 최선이 14곳 × 3.8 ≈ 53초, 신규가 흩어지면 그보다 는다.
//         최악(68건이 전부 다른 청크)은 68 × 3.8 ≈ 258초로 예산 240초를 넘는다.
//   그래서 판정 청크 사이에도 예산 가드를 뒀다(run-ingest.ts filterByTopic).
//         넘긴 몫은 `budget.skippedTopicChecks` 로 리포트에 남는다.
// 전에 "29분"이라던 수치는 **첫 수집 450건을 순차로** 돌렸을 때의 값이라 정상 주기의 비용이
// 아니었다 — 조건을 안 적어 둔 탓에 "판정을 켜지 말라"는 근거로 3일간 쓰였다.
export const SOURCES: readonly Source[] = [
  ...
  // ── 국내 언론 (한국어라 제목 번역이 필요 없다) ──
  {
    ...
    needsTopicCheck: true,
    tier: "daily",
    // pubDate 가 `2026-08-26 17:50:06` — 타임존이 없는 한국시간이다(2026-08-26 실측).
    // 안 적으면 UTC 로 읽혀 9시간 미래가 된다.
    feedTimezone: "Asia/Seoul",
  },
  {
    id: "aitimes-kr",
    name: "인공지능신문",
