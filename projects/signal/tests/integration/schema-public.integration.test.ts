...
describe("스키마가 실제로 올라갔는지", () => {
  it("INV-DA1 (S2): item · tag · item_tag 세 테이블이 있다 — 공개 키로 읽힌다", async () => {
...
describe("쓰기는 열려 있지 않다 (INV-S4 인접)", () => {
  ...
  it("INV-DA2 (S4, 실패경로): publishable 키로 insert 하면 RLS 가 막는다", async () => {
...
describe("ingest_run — 개발자용 대시보드 테이블은 공개 키로 안 보인다 (2026-08-17)", () => {
  it("INV-DA3 (S5, 실패경로): publishable 키로 select 하면 에러 없이 0행이다 (select 정책 자체가 없다)", async () => {
    // RLS 는 켜져 있고 정책이 없다 — insert 처럼 42501 로 거절하는 게 아니라
    // 조용히 빈 배열을 준다. "에러가 안 났다"만 보면 이게 공개로 보인다는 뜻으로 오독할 수 있어
    // 행 개수까지 같이 본다.
    const { data, error } = await anon.from("ingest_run").select("id").limit(1);
    expect(error).toBeNull();
    expect(data).toEqual([]);
  });

  it("item 에 last_ingest_run_id 컬럼이 실제로 올라가 있다", async () => {
    const { error } = await anon.from("item").select("last_ingest_run_id").limit(1);
    expect(error, error?.message).toBeNull();
  });
});
