...
/**
 * 본문에 제어문자가 섞였나. 데이터베이스의 `chat_messages_content_no_control` 과 같은
 * 판정이고, **강제 위치는 그쪽이다** — 여기는 사용자가 읽을 수 있는 문구를 주기 위한
 ...
 * **가운데**의 제어문자는 그대로 데이터베이스로 가고, 제약이 23514 로 거부하면
 ...
 *
 * 범위는 `[[:cntrl:]]` 와 같게 맞춘다(U+0000–1F · U+007F · U+0080–9F). 여기가 더 좁으면
 * 그 틈으로 들어온 값이 다시 위의 반복되는 오류가 되고, 더 넓으면 데이터베이스가 받아 줄
 * 메시지를 앱이 거절한다.
 */
