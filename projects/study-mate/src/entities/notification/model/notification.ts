...
/**
 * 알림 문장을 제목과 나머지로 나눠 준다. **한 문자열로 합치지 않는 이유가 둘**이다 —
 * 화면이 제목만 굵게 그려야 하고, 제목이 **남이 정한 문자열**이라 제품이 쓴 꼬리말과
 * 갈려 보여야 한다(화면이 낫표로 두른다 — `shared/lib/quote.ts`).
 *
 * 두르는 것은 여기가 아니라 화면이 한다. 값에 섞으면 다음 화면이 다시 두르면서
 * 「「제목」」이 되고, 알림은 저장된 값 그대로를 그리는 것이 계약이다(INV-N6).
 */
