  ...
  const 문장 = (
    <>
      ...
      <p className={styles.text}>
        {open}
        {/* **`<bdi>` 다 — `<b>` 가 아니다** (INV-T3). 굵기는 `.name` 이 주므로 보이는
            것은 그대로이고, 바뀌는 것은 방향 계산이 이 요소에서 닫힌다는 것이다.
            `<b>` 였을 때는 제목에 든 U+202E 하나가 **문단 끝까지** 가서 시각 순서가
            「꼬리말」제목 으로 뒤집혔다(2026-09-11 Chromium 실측).
            0022 가 그런 제목의 저장을 막지만 **알림의 제목은 사건 시점의 사본**이라
            제약이 생기기 전에 복사된 행은 원본을 고쳐도 안 바뀐다 — 저장을 막는 것과
            이미 저장된 값이 이 문단에서 할 수 있는 일을 막는 것은 다른 일이다. */}
        <bdi className={styles.name}>{body}</bdi>
        {close}
        {tail}
      </p>
