...
- **아직 안 정한 것**: 정확한 모델 이름과 SDK 버전은 만들 때 Context7 로 확인해서 쓴다 —
  기억으로 적은 설정 키는 조용히 틀린다.

### 서버 전용 비밀 키가 이 앱에 처음 들어온다

지금 study-mate 는 **서버에서도 공개 키만 쓴다**(아래 「환경 변수」). 그 사실이
`docs/specs/write-authorization.md` 의 전제로 적혀 있다. AI 키는 브라우저에 못 내보내므로
AI 국면에서 서버 전용 시크릿이 처음 생기고, 그때 그 스펙의 전제 문장을 다시 봐야 한다.
`[결정 2026-09-10]`
