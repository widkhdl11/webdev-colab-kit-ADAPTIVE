...
```
킷을 프로젝트 루트에 복사 → 킷 폴더 안에서 claude 실행 → "이런 웹사이트를 만들고 싶어"
```

> **반드시 킷 폴더 안에서 세션을 연다.** 브리핑·게이트·훅은 전부 `.claude/settings.json` 에 등록돼 있고,
> 그 파일은 세션을 연 폴더 기준으로 읽힌다. 상위 폴더에서 열면 **파일은 멀쩡한데 아무것도 안 실린다** —
> 보호 파일이 그냥 편집되고 Stop 훅도 안 돈다. 2026-08-15 에 실제로 그 상태로 한 세션을 보냈고,
> 게이트·그래프·훅이 전부 무방비였다(`docs/references/harness-backlog.md` 참조).
