...
수동 진입도 있다: `node gates/graph-stop.mjs --mark <노드>` [(graph-stop.mjs:193-206)](gates/graph-stop.mjs#L193-L206).

## What it does
1. **게이트**: `run-gates` 전체를 자식 프로세스로 돌리고 에러를 `{cat, relPath, whole}` 로 파싱한다 [(graph-stop.mjs:210-211, 130-149)](gates/graph-stop.mjs#L210-L211).
2. **상태 로드**: HANDOFF.md 의 ```json 블록을 읽고, 그래프에 있는데 상태에 없는 노드/자식을 dirty 로 백필한다 [(graph-stop.mjs:88-101)](gates/graph-stop.mjs#L88-L101).
3. **sync**: 각 단위의 `produces` 내용 해시(sha256 앞 12자)를 계산해 이전과 다르면 dirty 로 마크하고 전파한다. 사인오프 노드는 제외 [(graph-stop.mjs:65-72, 216-222)](gates/graph-stop.mjs#L65-L72).
4. **release**: 위상정렬 순서로, 상류가 전부 clean 이고 프론트매터·exists_nonempty 를 만족하며 해당 카테고리 게이트 에러가 0건인 dirty 단위를 clean 으로 내린다 [(graph-stop.mjs:226-247)](gates/graph-stop.mjs#L226-L247).
5. **집계**: 병렬 부모(design)는 자식 처리 직후 즉시 확정한다 — 하류가 그 값을 보고 판정하기 때문이다 [(graph-stop.mjs:243-247)](gates/graph-stop.mjs#L243-L247).
6. **persist**: 프론티어를 주석 줄에, 상태를 JSON 블록에 써서 HANDOFF.md 를 갱신한다 [(graph-stop.mjs:107-116)](gates/graph-stop.mjs#L107-L116).
7. **안내**: qa 실패면 분류기 필요 신호, 사인오프 노드가 프론티어면 basis 해시를 알려준다 [(graph-stop.mjs:252-263)](gates/graph-stop.mjs#L252-L263).
8. run-gates 가 exit 2 였으면 마지막에 자신도 exit 2 로 차단한다 [(graph-stop.mjs:266-269)](gates/graph-stop.mjs#L266-L269).
...
- 경로 있는 에러 → produces 글롭에 매칭되는 노드만 차단 [(graph-stop.mjs:169-176)](gates/graph-stop.mjs#L169-L176)
- 프로젝트 통째 에러 → `whole: true` 로 해당 카테고리를 요구하는 노드 전부 차단
- 사인오프 노드 → 마커 + basis 해시 [(graph-stop.mjs:179-188)](gates/graph-stop.mjs#L179-L188)

## Failure path
- run-gates 실패가 남아 있으면 exit 2 로 턴을 차단한다(기존 동작 유지).
- HANDOFF.md 의 JSON 이 깨지면 `catch` 후 빈 상태에서 재부트스트랩한다 [(graph-stop.mjs:92-94)](gates/graph-stop.mjs#L92-L94).
