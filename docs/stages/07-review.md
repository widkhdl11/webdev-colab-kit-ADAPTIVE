...
| `security-reviewer` | 입력·인가·시크릿·세션을 만진 diff. **auth·payment·authz 표면이 코드에 있으면 사인오프가 이 리뷰어를 요구한다** | [CONFIRMED: CLAUDE.md:145 / graph.mjs:110] |
...
## Gate
`clean_when.signoff` 에 marker·require·basis_of + `reviewers_field`·`require_reviewer` 가 선언돼 있다 [(graph.mjs:104-113)](../../graph.mjs#L104-L113).
...
- **code/ui/test-auditor 파견 여부** — 기계가 볼 수 있는 표면 신호가 없어 판단으로 남겼다. 강제되는 것은 security-reviewer 뿐이다. [CONFIRMED: graph.mjs — require_reviewer 에 security-reviewer 만 선언 — graph.mjs:110]
