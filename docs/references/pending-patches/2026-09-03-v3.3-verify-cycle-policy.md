...
export const POLICY_SCOPES = ["ui", "data-model", "api", "copy"];      // ← 이 줄만 바꾼다
...
보류 신고 블록이 끝나고 `if (errors.length > 0) {` 가 시작되는 자리다(710번째 줄 근처).
...
### ④-1 lib 을 부른다 (import 줄 바로 아래, 18번째 줄 근처)
...
### ④-2 종료 판정과 문구 (483~508번째 줄 근처)

**여기는 한 덩어리로 바꾼다.** 조각조각 바꾸면 마지막 `});` 가 같이 지워지기 쉽고, 그러면
파일 전체가 문법 오류가 되어 Stop 훅이 매 턴 죽는다 — 2026-09-04 에 실제로 그랬다.
아래 "바꾸기 전" 전체를 "바꾼 뒤" 전체로 갈아 끼운다. 첫 줄(`}`)과 마지막 줄
(`const rw = ...`)은 위치를 잡기 위한 것이고 내용은 그대로다.
...
**바꾼 뒤** (바뀐 것은 함수 이름, 종료 판정, 마지막 두 메시지뿐이다 — `});` 는 그대로 있다):
