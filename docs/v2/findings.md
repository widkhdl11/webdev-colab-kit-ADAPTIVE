...
> **재현 명령을 지금 그대로 돌리면 안 된다.** `F-11` · `F-12` 의 재현 명령이 가리키는
> `projects/signal/` 은 2026-08-31 에 레포에서 지웠고(자리를 물려받은 `projects/signal2/` 는 아직
> `src/` 가 없다), 그래서 두 명령 다 **빈 결과**를 낸다. 빈 결과는 결함이 고쳐졌다는 뜻이 아니다 —
> [graph.mjs:96](../../graph.mjs#L96) 은 여전히 `src/**/*.test.ts` 이고 [:77](../../graph.mjs#L77) 은
> 여전히 `src/entities/*/model.ts` 라, 둘 다 그대로 열려 있다.
> 그때의 파일에 대고 재현하려면 `git checkout ccc8edc -- projects/signal` 로 꺼내서 돌린다.
